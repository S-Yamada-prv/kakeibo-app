...
const defaultCategories = ['食費', '日用品', '外食', '交通費', '医療費', 'その他'];
...
const canAnalyze = Boolean(import.meta.env.VITE_API_URL) || import.meta.env.DEV;
...
function App() {
  ...
  const [categories, setCategories] = useState(() => {
    try { return JSON.parse(localStorage.getItem('receipt-ledger-categories')) || defaultCategories; } catch { return defaultCategories; }
  });
  ...
  useEffect(() => localStorage.setItem('receipt-ledger-categories', JSON.stringify(categories)), [categories]);
  ...
  async function analyzeReceipt(file) {
    ...
    try {
      const response = await fetch(`${apiUrl}/analyze-receipt`, { method: 'POST', body: formData });
  ...
  function saveEdit() {
    ...
    if (!draft?.date || !draft.store || !draft.name || draft.amount === '' || !Number.isFinite(amount)) {
      setMessage('日付、店舗名、商品名、金額を入力してください。');
      return;
    }
    setRecords((current) => current.map((record) => {
      if (record.id !== editing.recordId) return record;
      return { ...record, date: draft.date, store: draft.store, items: record.items.map((item, index) => index === editing.itemIndex ? { name: draft.name, amount, category: draft.category, lineType: amount < 0 ? 'discount' : 'purchase', confidence: 1, needsReview: false } : item) };
    }));
  ...
  function addCategory(event) {
    ...
    if (categories.includes(category)) {
      setMessage('そのカテゴリはすでに登録されています。');
      return;
    }
    setCategories((current) => [...current, category]);
    setNewCategory('');
    setMessage(`カテゴリ「${category}」を追加しました。`);
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      <section className="hero"><div><p className="eyebrow accent">レシートから、暮らしの輪郭を。</p><h2>買い物を記録して、<br /><em>お金の流れ</em>を眺める。</h2><p className="hero-copy">画像をアップロードするだけで、購入品を自動で整理。いつもの支出を、静かに見える化します。</p></div><div className={`drop-zone ${isDragging ? 'dragging' : ''}`} onDragEnter={(event) => { event.preventDefault(); setIsDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop}><button className="upload-button" onClick={() => fileInput.current?.click()} disabled={!canAnalyze || isAnalyzing}><span className="upload-icon">{isAnalyzing ? '…' : '＋'}</span><span>{isAnalyzing ? '読み取り中' : canAnalyze ? 'レシートを読み込む' : '解析API未接続'}</span><small>{canAnalyze ? 'クリックまたは画像をドロップ / JPG・PNG・WEBP' : '手入力で記録を追加できます'}</small></button><input ref={fileInput} type="file" accept="image/jpeg,image/png,image/webp,image/gif" hidden onChange={(event) => selectFile(event.target.files?.[0])} /></div></section>
