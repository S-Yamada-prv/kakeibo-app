...
function App() {
  ...
  function saveEdit() {
    ...
    setRecords((current) => current.map((record) => {
      if (record.id !== editing.recordId) return record;
      return { ...record, date: draft.date, store: draft.store, items: record.items.map((item, index) => index === editing.itemIndex ? { name: draft.name, amount, category: draft.category } : item) };
    }));
